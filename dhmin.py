...
def create_model(vertex, edge, params=None, timesteps=None, shapefiles=None):
    ...
    tech_parameters = {
        ...
        'c_rev': 0.07,  # (€/kWh) retail price for heat
    ...
    m.k_heatvar = pyomo.Param(m.vertex, initialize=dict(
        vertex['c_heatvar'] / tech_parameters['concurrence']
    ))

    # k_heatfix: costs for heat generation (EUR/kW)
    m.k_heatfix = pyomo.Param(m.vertex, initialize=dict(
        vertex['c_heatfix'] * tech_parameters['annuity']
    ))
...
def cost_rule(m, cost_type):
    if cost_type == 'network':
        ...
        return m.costs['heat'] == \
               sum(m.k_heatfix[i] * m.Q[i, t] + m.k_heatvar[i] * m.Q[i, t] * m.dt[t]
                   for i in m.vertex
