...
This module contains helper functions for pre- and postprocessing data for and
...
def plot_flows_min(model: Any) -> None:
    ...
    for i, (name, group) in enumerate(power_flows_grouped):
        ...
        plt.xlim(0, 10)
        plt.ylim(0, 10)
        for key, value in group["Pin"].items():
        ...
        for key, value in q.items():
            ...
            plt.scatter(
                x_pos, y_pos, zorder=z, s=w, facecolors=colour, edgecolors="none", linewidths=0
            )
    plt.show()
...
def symmetrize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_symmetric: pd.DataFrame = df.add(df_tmp, fill_value=0)
    return df_symmetric
