...
def create_model(
    ...
    timesteps: list[tuple[int, float]] | None = None,
    edge_profile: pd.Series | None = None,  # type: ignore[type-arg]
    name: str = "DHMIN",
) -> pyomo.ConcreteModel:
    ...
    Args:
        ...
        edge_profile: per-edge time profile series
    ...
    tech_parameters: dict[str, float] = {
        "c_fix": 600,  # (EUR/m) fixed pipe investment
        "c_var": 0.015,  # (EUR/kW/m) variable pipe investment
        "c_om": 5,  # (EUR/m) operation & maintenance
        "c_rev": 0.07,  # (EUR/kWh) retail price for heat
    ...
    edges = pd.concat([edges_tmp, edges], verify_integrity=True)
    del edges_tmp

    # derive list of neighbours for each vertex
    neighbours: dict[Any, list[Any]] = {}
    ...
    ts_list: list[tuple[str, int, float]]
    availability: npt.NDArray[np.int_]
    ...
    if timesteps:
        ...
        ts_list = [(f"t{t[0]}", t[0], t[1]) for t in timesteps]
        ts_list.append(("Pmax", 1, 1))
        ...
        availability = np.ones(
            (len(ts_list) + len(source_vertex), len(source_vertex)),
            dtype=np.int_,
        )

        for i, v0 in enumerate(source_vertex):
            availability[len(ts_list), i] = 0
            ts_list.append((f"v{v0}", 1, 1))
    else:
    ...
    if edge_profile is not None:
        ...
        ep = edge_profile.apply(
            lambda x: [(f"t{duration}", duration, sf) for duration, sf in x]
        )
    ...
    m.timesteps = pyomo.Set(initialize=[t[0] for t in ts_list])
    ...
    m.delta = pyomo.Param(
        m.edge,
        initialize=dict(
            edges["peak"] * edges["cnct_quota"] * tech_parameters["concurrence"]
            + edges["length"] * tech_parameters["thermal_loss_fix"]
        ),
    )
    m.eta = pyomo.Param(
        m.edge,
        initialize=dict(1 - (edges["length"] * tech_parameters["thermal_loss_var"])),
    )
    ...
    # k_heatvar: costs for heat generation (EUR/h)
    ...
    m.k_heatvar = pyomo.Param(
        m.vertex,
        initialize=dict(vertex["c_heatvar"] / tech_parameters["concurrence"]),
    )

    # k_heatfix: costs for heat generation (EUR/kW)
    m.k_heatfix = pyomo.Param(
        m.vertex,
        initialize=dict(vertex["c_heatfix"] * tech_parameters["annuity"]),
    )

    # r_heat: revenue for heat delivery (EUR/h)
    m.r_heat = pyomo.Param(
        m.edge,
        initialize=dict(
            edges["peak"]
            * 0.5  # x and Pmax are forced in both directions (i,j),(j,i)
            * edges["cnct_quota"]
            * tech_parameters["c_rev"]
        ),
    )
    m.availability = pyomo.Param(
        m.source_vertex,
        m.timesteps,
        initialize={
            (s, t[0]): availability[x, y]
            for y, s in enumerate(source_vertex)
            for x, t in enumerate(ts_list)
        },
    )
    m.dt = pyomo.Param(
        m.timesteps,
        initialize={ts_name: duration for ts_name, duration, _scaling_factor in ts_list},
    )

    m.scaling_factor = pyomo.Param(
        m.edge,
        m.timesteps,
        initialize={
    ...
    m.x = pyomo.Var(m.edge, within=pyomo.Binary)  # Is pipe built?
    ...
    m.y = pyomo.Var(m.edge, m.timesteps, within=pyomo.Binary)  # Is pipe used during timestep

    m.energy_conservation = pyomo.Constraint(
        m.vertex,
        m.timesteps,
        doc="Power flow is conserved in vertex",
        rule=_energy_conservation_rule,
    )
...
def _energy_conservation_rule(m: pyomo.ConcreteModel, i: Any, t: Any) -> Any:
    return sum(m.Pin[i, k, t] - m.Pot[k, i, t] for k in m.neighbours[i]) <= m.Q[i, t]


def _demand_satisfaction_rule(m: pyomo.ConcreteModel, i: Any, j: Any, t: Any) -> Any:
    return (
        m.Pot[i, j, t]
        == m.Pin[i, j, t] * m.eta[i, j] - m.y[i, j, t] * m.delta[i, j] * m.scaling_factor[i, j, t]
    )


def _pipe_capacity_rule(m: pyomo.ConcreteModel, i: Any, j: Any, t: Any) -> Any:
    return m.Pin[i, j, t] <= m.Pmax[i, j]


def _pipe_usage_rule(m: pyomo.ConcreteModel, i: Any, j: Any, t: Any) -> Any:
    return m.Pin[i, j, t] <= m.y[i, j, t] * m.edges.loc[i, j]["cap_max"]


def _must_build_rule(m: pyomo.ConcreteModel, i: Any, j: Any) -> Any:
    return m.x[i, j] >= m.edges.loc[i, j]["must_build"]


def _build_capacity_rule(m: pyomo.ConcreteModel, i: Any, j: Any) -> Any:
    return m.Pmax[i, j] <= m.x[i, j] * m.edges.loc[i, j]["cap_max"]


def _unidirectionality_rule(m: pyomo.ConcreteModel, i: Any, j: Any, t: Any) -> Any:
    return m.y[i, j, t] + m.y[j, i, t] <= 1


def _symmetry_x_rule(m: pyomo.ConcreteModel, i: Any, j: Any) -> Any:
    return m.x[i, j] == m.x[j, i]


def _symmetry_pmax_rule(m: pyomo.ConcreteModel, i: Any, j: Any) -> Any:
    return m.Pmax[i, j] == m.Pmax[j, i]


def _built_then_use_rule(m: pyomo.ConcreteModel, i: Any, j: Any, t: Any) -> Any:
    return m.y[i, j, t] + m.y[j, i, t] >= (m.x[i, j] + m.x[j, i]) / 2


def _source_vertices_rule(m: pyomo.ConcreteModel, i: Any, t: Any) -> Any:
...
def _cost_rule(m: pyomo.ConcreteModel, cost_type: str) -> Any:
    if cost_type == "network":
        return m.costs["network"] == sum(
            m.k_fix[i, j] * m.x[i, j] + m.k_var[i, j] * m.Pmax[i, j] for (i, j) in m.edge
        )
    elif cost_type == "heat":
        return m.costs["heat"] == sum(
            m.k_heatfix[i] * m.Q[i, t] + m.k_heatvar[i] * m.Q[i, t] * m.dt[t]
            for i in m.vertex
            for t in m.timesteps
        )
    elif cost_type == "revenue":
        return m.costs["revenue"] == -sum(
            m.r_heat[i, j] * m.x[i, j] * m.scaling_factor[i, j, t] * m.dt[t]
            for (i, j) in m.edge
            for t in m.timesteps
        )
    else:
        raise NotImplementedError(f"Unknown cost type: {cost_type!r}")


def _obj_rule(m: pyomo.ConcreteModel) -> Any:
    return sum(m.costs[cost_type] for cost_type in m.cost_types)


def anf(n: int, i: float) -> float:
...
def get_entity(instance: pyomo.ConcreteModel, name: str) -> pd.DataFrame:
    ...
    if isinstance(entity, pyomo.Set):
        ...
        if entity.dim() > 1:
            ...
            results = pd.DataFrame([v[0] + (v[1].value,) for v in entity.items()])
        else:
...
def get_entities(instance: pyomo.ConcreteModel, names: list[str]) -> pd.DataFrame:
    ...
    for entity_name in names:
        other = get_entity(instance, entity_name)
...
def list_entities(instance: pyomo.ConcreteModel, entity_type: str) -> pd.DataFrame:
    ...
    def filter_by_type(entity: Any, entity_type: str) -> bool:
        if entity_type == "set":
    ...
    # iterate through all model components and keep only matching ones
    iter_entities = instance.__dict__.items()
    entities_list = sorted(
        (ename, entity.doc, _get_onset_names(entity))
        for (ename, entity) in iter_entities
        if filter_by_type(entity, entity_type)
    )

    # if something was found, wrap tuples in DataFrame, otherwise return empty
    if entities_list:
        entities = pd.DataFrame(entities_list, columns=["Name", "Description", "Domain"])
...
def _get_onset_names(entity: Any) -> list[str]:
