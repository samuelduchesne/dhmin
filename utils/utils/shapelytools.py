...
# <change>
def snapping_vertexis_from_lines(lines, closeness_limit):
    """" Return a list of edge endpoints, where close, but not identical
         points are filtered

    Args:
        lines: list of LineStrings
        closeness_limit: critical distance for selecting uniqe verteces
    """
    uni_verts = []
    for line in lines:
        for end in line.coords:
            if len(uni_verts):
                inter = Point(end).buffer(closeness_limit).intersection(MultiPoint(uni_verts))
                if inter.type != 'Point':
...
def snappy_endings(lines, max_distance):
    ...
    # </old>
    # <change>
    # snapping_points = snapping_vertexis_from_lines(snapped_lines, max_distance)
    # </change>
